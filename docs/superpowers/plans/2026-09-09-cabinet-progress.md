...
| F4 Service/restricted profiles | IN_PROGRESS | `f85ea0b` | F4a committed `f85ea0b`, review fixes in the follow-up commit; F4b pending. 121 profile/process/hook tests pass, 307 across the suite (unit_verified). The launch command line is derived from the profile and re-derived on verification, so tampering with argv alone is refused; the credential rule denies `~/.cabinet/**/runtime` and the private parts of `~/.claude` while leaving the company's views and the installed plugin readable; worker settings register the dispatch check because a worker loads no plugin; the profile carries the environment that check reads. **O1 obligation:** the packaged advisory agent files (`brand`, `cfo`, `counsel`, `qa`, `delivery-lead`, `architect`) still declare `WebSearch`/`WebFetch`/`mcp__github__*` and no `Skill`/`SendMessage`/`ListAgents`, which contradicts the staff tool set the registry now defines. O1 rewrites them and lands the frontmatter-versus-registry structural test. Nothing here is native_verified: the MCP service, launcher and live isolation smoke are F4b |
