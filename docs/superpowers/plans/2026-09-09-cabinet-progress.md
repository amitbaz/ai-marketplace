...
| Restricted launch capability handshake | supported (native_verified) | A `claude -p` session started from `cabinet-launch`'s own MCP configuration reported `launch: restricted` and took the company lease at generation 2 under the launcher's session id |
