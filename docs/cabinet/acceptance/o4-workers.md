...
### Blocker 1: Superset Live Gate — Owner Authentication → project on host
**Status:** Authentication CLOSED; a project now exists (`cabinet-fixture`, id `34da8ba1-4f8a-4650-8cd8-4d92a4309697`) but is not adopted on this host — BLOCKED on an owner action, adapter defect found and fixed along the way
**Detail:** `superset auth whoami --json` succeeds. `SupersetAdapter.probe()` run live: `usable: True`. The owner then added project `cabinet-fixture` (id above, `main_repo_path /Users/amitbaz/.superset/projects/cabinet-fixture`). This host is not cloud-registered (`superset status` → `cloudRegistered: false`, `superset hosts list --json` → `[]`), so `superset projects list --json` still returns `[]` — but the project id itself is usable directly (`superset workspaces list --project <id> --json` → `[]`, no error).

**Adapter defect found and fixed:** `SupersetAdapter.audit_setup_isolation()` called a nonexistent CLI subcommand (`projects get`) and raised `PROVIDER_ERROR: ... Unknown command: get` instead of failing cleanly — Superset 1.27.0's `projects` command only has `create`/`list`/`setup`. Fixed to read `projects list --local --json` and match by id (same pattern `reconcile_assignment` already uses for workspaces); a project absent from that list now returns a clean `{'contained': False, 'reason': 'project <id> is not set up on this host ...'}` instead of raising. Full trace in `docs/cabinet/acceptance/environment.md` (Addendum "Superset project discovery"). Unit tests: `tests/cabinet/test_superset.py::SupersetArgvTest.test_audit_reads_projects_list_not_a_get_subcommand`, `test_a_project_not_set_up_on_this_host_is_not_contained`, `test_a_project_with_a_setup_command_is_not_contained`.

**With the fix, the audit correctly refuses before any workspace is created:** `audit_setup_isolation()` on the real project id returns `contained: False` (not adopted here). `superset workspaces create --project <id> ... --local --json` was also tried directly and refused verbatim: `Error: Project is not set up on this host` — the same gap, consistent with the audit result. `service.py`'s `SETUP_ISOLATION_UNAVAILABLE` gate calls `audit_setup_isolation()` before any workspace is created, so a real worker launch would correctly refuse here rather than attempt `workspaces create` and hit the same wall.

**Next action (owner, not run here):** adopt the project on this host (`superset projects setup --project 34da8ba1-4f8a-4650-8cd8-4d92a4309697 --import /Users/amitbaz/.superset/projects/cabinet-fixture` or equivalent) or register the host to the cloud (`superset start --org …`). Once either is done, re-run A.3: create one workspace + terminal through the real adapter, launch a `--model haiku` worker, observe registration with a real chief, an Engineering→worker message and reply, close the terminal, `claude rm`.
