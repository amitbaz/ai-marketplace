...
| Subagent → parent-session delivery | supported (executed, confirmed by the recipient) | a subagent called `SendMessage(to: "main", …)` | The parent session confirmed receiving `<agent-message from="aa3fd74e12b29d52e">` with the body `PROBE-TOKEN-8F31C`. It landed in the parent's main conversation, not in the spawning teammate's. The `from` attribute carries the sender's agent name or teammate name; the peer address shown to a human is the **parent session's** name. |
