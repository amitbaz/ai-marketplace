...
6. **Idle wake works, and only the chief can arm it.** A subagent cannot
   ...
   peer was a child process of the subscriber's shell. O2 must re-probe with a
   Superset-launched worker before relying on it across a real containment
   boundary.
