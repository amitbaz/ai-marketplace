...
`.muster/money.md`, and every notebook. Missing charter: say so, suggest
`/muster:hire`, and stop.
...
## 3. Re-check what the stage conditioned
...
attributed. Numbers only ever go up: never renumber an open item, never reuse
a closed one's number. Add `## MONEY` items to `.muster/money.md` under open, updating
days-open rather than duplicating anything already there.
